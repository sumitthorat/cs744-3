...
#define GET_HEIGHT(ptr) (*((WPTR)ptr + 4))
...
#define MIN_FREE_BLOCK_SIZE 24 // 16 = 4W = HDR + FTR + NEXT + LEFT + RIGHT + HEIGHT 

// Get balance factor of the Node
#define GET_BALANCE(ptr) (ptr ? GET_HEIGHT(GET_LEFT(ptr)) - GET_HEIGHT(GET_RIGHT(ptr)) : 0)
...
WPTR remove_block(WPTR node, WPTR ptr, WPTR parent) {
	...
	if (block_size < GET_BLOCK_SIZE(node)) 
		...
		if (!GET_NEXT(node)) { // There is more than 1 block with the size same as this block. So node deletion is not required from tree
			...
			else { // The block we are searching for is in the list
				WPTR prev = node;
				WPTR curr = GET_NEXT(node);
				while(curr != ptr) {
					prev = curr;
					curr = GET_NEXT(curr);
				}
			...
			if (!GET_LEFT(node) && !GET_RIGHT(node)) // no child
				return NULL;
